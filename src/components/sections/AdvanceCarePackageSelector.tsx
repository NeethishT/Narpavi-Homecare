...
export default function AdvanceCarePackageSelector() {
  ...
  const handleBookNow = (packageName: string) => {
    ...
    document.getElementById('advance-nursing-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    window.setTimeout(() => {
      document.querySelector<HTMLInputElement>('#advance-nursing-form .cta-form input[name="name"]')?.focus({ preventScroll: true });
    }, 450);
  };
      ...
      <div className="elder-package-list" aria-live="polite">
        ...
        >
          <div className="basic-care-package-card__top">
            ...
            <button type="button" className="btn btn--primary btn--sm" onClick={() => handleBookNow(selectedPackage.name)}>Book Advanced Nursing Care</button>
          </div>
          ...
          <div className="basic-care-package-card__best">
            ...
            <div className="basic-care-package-card__media">
              <Image src={selectedPackage.image} alt={`${selectedPackage.name} Advanced Nursing Care package`} fill sizes="(max-width: 992px) 100vw, 220px" style={{ objectFit: 'cover' }} />
            </div>
          </div>
